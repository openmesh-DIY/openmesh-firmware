...
#define MAX_TTL 8
#define LORA_SYNCWORD 0x12   // DO NOT CHANGE unless all nodes do
...
// AES-256 (static, shared, intentional)
...
void generateAutoName() {
    ...
    nodeName =
      String(adj[chipId % 7]) + "-" +
      String(noun[(chipId >> 8) % 7]) + "-" +
      String(chipId & 0xFF, HEX);
...
void handleBT() {
    ...
    if (cmd.startsWith("/sf ")) {
        currentCfg.sf = cmd.substring(4).toInt();
        applyLoRa();
    }
    else if (cmd.startsWith("/bw ")) {
        currentCfg.bw = cmd.substring(4).toInt();
        applyLoRa();
    }
    else if (cmd.startsWith("/tx ")) {
        currentCfg.tx = cmd.substring(4).toInt();
        applyLoRa();
    }
    else if (cmd == "/id") {
        SerialBT.printf(
...
void drawUI() {
    ...
    if (uiState == UI_MSG_POPUP) {
        u8g2.setCursor(0,10);
        u8g2.print("INCOMING MSG:");
        u8g2.setCursor(0,30);
        u8g2.print(lastReceivedMsg);
    }
...
void longPress() {
    if (uiState == UI_WORD) uiState = UI_DEST;
        ...
        uint16_t d =
          (destIndex == knownCount)
          ? BROADCAST_ID
          : knownNodes[destIndex];
...
void loop() {
    ...
    if (h.dest == nodeID || h.dest == BROADCAST_ID) {
        ...
        lastReceivedMsg = String((char*)dec);
        uiState = UI_MSG_POPUP;
        msgDisplayTimer = millis();
        drawUI();
